...
void Snake::Extend()
{
	...
	SnakeSegment& tail_head = m_snakeBody[m_snakeBody.size() - 1]; // Create a reference to the last element in the segment vector

	if (m_snakeBody.size() > 1)
	{
		// If the snake is longer than one segment, create a reference to the next to last element
